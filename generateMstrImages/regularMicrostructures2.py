...
class SingleParamMicrostructures:
    ...
    def vbox(self, t):
        ...
        for rw in range(self.nelx):
            for col in range(self.nely):
                # Bar
                if col > 0.5 * self.nely - 0.5 * t and col < 0.5 * self.nely + 0.5 * t:
                    microStr[rw, col] = 1
                # Box
                if (rw < t) or (self.nelx - rw < t) or (col < t) or (self.nely - col < t):
                    microStr[rw, col] = 1
    ...
    def hbox(self, t):
        ...
        for rw in range(self.nelx):
            for col in range(self.nely):
                # Bar
                if rw > 0.5 * self.nelx - 0.5 * t and rw < 0.5 * self.nelx + 0.5 * t:
                    microStr[rw, col] = 1
                # Box
                if (rw < t) or (self.nelx - rw < t) or (col < t) or (self.nely - col < t):
                    microStr[rw, col] = 1
        ...
        for rw in range(self.nelx):
            ...
            for col in range(self.nely):
                ...
                if (rw < boxt) or (self.nelx - rw < boxt) or (col < boxt) or (self.nely - col < boxt):
                    microStr[rw, col] = 1
                # Bar
                if rw > 0.5 * self.nelx - 0.5 * boxt and rw < 0.5 * self.nelx + 0.5 * boxt:
                    microStr[rw, col] = 1

        return microStr

    # Microstructure number 10
    def diamVBox(self, t):
        microStr = self.eps * np.ones((self.nelx, self.nely))
        delt = 0.3 * t
        boxt = 0.4 * delt * self.nelx
        for rw in range(self.nelx):
            ...
            for col in range(self.nely):
                ...
                if (rw < boxt) or (self.nelx - rw < boxt) or (col < boxt) or (self.nely - col < boxt):
                    microStr[rw, col] = 1
                # Bar
                if col > 0.5 * self.nely - 0.5 * boxt and col < 0.5 * self.nely + 0.5 * boxt:
                    microStr[rw, col] = 1
        return microStr

    # Microstructure number 6
...
def test():
    ...
    # Create an intansce of the microstructure creator
    M = SingleParamMicrostructures(nelx, nely)
    ...
    # Lambda function of microstructures
    mstrFn = {'0': lambda t: M.square(t),
              '1': lambda t: M.Xbox(t),
              '2': lambda t: M.X(t),
